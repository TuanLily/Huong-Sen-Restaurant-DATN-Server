...
router.post("/", (req, res) => {
  ...
  const sql = `INSERT INTO reservations 
                 (reservation_code, user_id, promotion_id, fullname, tel, email, reservation_date, party_size, note, total_amount, deposit, status) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      ...
      if (promotion_id) {
        ...
        connection.query(updatePromotionSql, [promotion_id], (err) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Failed to update promotion quantity" });
          }
          res
            .status(201)
            .json({
